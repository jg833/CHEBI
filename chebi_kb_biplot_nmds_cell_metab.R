...
colours_lowcho <- c("#f4c16e","#EEA327","#ba790f")
# LOWCHO --------------------------------------------------------------------
...
vegan_nmds %>% stressplot()
vegan_nmds %>% ordiplot(display = 'sites',cex=1.25,air=0.01)
...
lowcho_plot <-
  ...
  stat_ellipse(level = 0.95, geom = 'polygon', aes(fill = combined), alpha = 0.25, show.legend = F) +
  geom_point(data = centroid, size = 3, shape = 21, colour = 'black', aes(fill = combined), show.legend = F) +
  geom_text(aes(x = NMDS1, y = NMDS2, label = sig),
            data = centroid,family = 'Helvetica', size = 2, inherit.aes = F)+
  # geom_text(aes(x = 15, y = 20, label = 'Stress = 0.226'),
  #           family = 'Helvetica', size = 3,inherit.aes = F)+
  scale_fill_manual(values = colours_lowcho)+ #,
                    # labels = c('Baseline', 'Week 4', 'Week 12'))+
  scale_colour_manual(values = colours_lowcho)+
  xlim(-20,20) +
  ylim(-20,20)+
  guides(colour = F)+
  # labs(colour = NULL, fill = 'Trial', family = 'Helvetica')+
  # ggtitle('LOWCHO')+
...
ggsave(filename = './Figures/chebi_kb_nmds_LOWCHO.tiff',
       device = 'tiff',
       width = 16,
       height = 8,
       units = 'cm',
...
modsug_plot <- nmds_samples %>%
  ...
  geom_text(aes(x = 15, y = 20, label = 'Stress = 0.153'),
...
# ggsave(filename = './Figures/chebi_nmds_MODSUG.tiff',
#        device = 'tiff',
#        width = 16,
#        height = 8,
#        units = 'cm',
...
lowsug_plot <- nmds_samples %>%
  ...
  stat_ellipse(level = 0.95) +
  geom_point(data = centroid, size = 5, shape = 21, colour = 'black', aes(fill = combined)) +
  geom_text(aes(x = 15, y = 20, label = 'Stress = 0.199'),
            family = 'Times', size = 3,inherit.aes = F)+
  scale_fill_futurama(labels = c('Baseline', 'Week 4', 'Week 12'))+
  scale_colour_futurama()+
  xlim(-20,20) +
...
# ggsave(filename = './Figures/chebi_nmds_LOWSUG.tiff',
#        device = 'tiff',
#        width = 16,
#        height = 8,
#        units = 'cm',
...
ggarrange(modsug_plot, lowsug_plot, lowcho_plot, nrow = 1, ncol = 3, common.legend = T, align = 'hv',
          labels = list('MODSUG','LOWSUG','LOWCHO'),
          font.label = list(family = 'Times', size = 10), hjust = -0.9)

ggsave(filename = './Figures/chebi_nmds_groups_arranged_landscape_pvals_stress.tiff',
       device = 'tiff',
       width = 25.7,
       height = 11.5,
       units = 'cm',
...
centroid <- nmds_samples %>% 
  group_by(combined) %>% 
  summarize(NMDS1 = mean(NMDS1), NMDS2 = mean(NMDS2)) %>% 
  mutate(group = str_replace_all(combined, '-A', '')) %>% 
...
baseline_plot <-
  ...
  theme(text = element_text(family = 'Helvetica',
        ...
        legend.background = element_blank(),
        legend.position = 'none')+
  # geom_text(aes(label = id_2),family = 'Helvetica', size = 3, check_overlap = F) +
  geom_point(size = 0.75, show.legend = F)+
  
  stat_ellipse(level = 0.95, geom = 'polygon', aes(fill = group), alpha = 0.1, show.legend = F) +
  geom_point(data = centroid, size = 3, shape = 21, colour = 'black', aes(fill = group), show.legend = F) +
  # geom_text(aes(x = 15, y = 20, label = 'Stress = 0.184'),
...
# ggsave(filename = './Figures/chebi_nmds_A.tiff',
#        device = 'tiff',
#        width = 16,
#        height = 8,
#        units = 'cm',
...
centroid <- nmds_samples %>% 
  group_by(combined) %>% 
  summarize(NMDS1 = mean(NMDS1), NMDS2 = mean(NMDS2)) %>% 
  mutate(group = str_replace_all(combined, '-B', '')) %>% 
  mutate(group = factor(group, levels = c('MODSUG', 'LOWSUG', 'LOWCHO')))
...
week_4_plot <- nmds_samples %>%
  ...
  stat_ellipse(level = 0.95, geom = 'polygon', aes(fill = group), alpha = 0.1, show.legend = F) +
  geom_point(data = centroid, size = 3, shape = 21, colour = 'black', aes(fill = group), show.legend = F) +
  # geom_text(aes(x = 15, y = 20, label = 'Stress = 0.191'),
  #           family = 'Helvetica', size = 2,inherit.aes = F)+
...
# ggsave(filename = './Figures/chebi_nmds_B.tiff',
#        device = 'tiff',
#        width = 16,
#        height = 8,
#        units = 'cm',
...
centroid <- nmds_samples %>% 
  group_by(combined) %>% 
  summarize(NMDS1 = mean(NMDS1), NMDS2 = mean(NMDS2)) %>% 
  mutate(group = str_replace_all(combined, '-C', '')) %>% 
  mutate(group = factor(group, levels = c('MODSUG', 'LOWSUG', 'LOWCHO'))) %>% 
  mutate(sig = c('#','',''))
...
week_12_plot <- nmds_samples %>%
  ...
  stat_ellipse(level = 0.95, geom = 'polygon', aes(fill = group), alpha = 0.1, show.legend = F) +
  geom_point(data = centroid, size = 3, shape = 21, colour = 'black', aes(fill = group), show.legend = F) +
  # geom_text(aes(x = 15, y = 20, label = 'Stress = 0.213'),
  #           family = 'Helvetica', size = 2,inherit.aes = F)+
  geom_text(aes(x = NMDS1, y = NMDS2, label = sig),
            data = centroid,family = 'Helvetica', size = 2, inherit.aes = F)+
...
ggarrange(baseline_plot, week_4_plot, week_12_plot, nrow = 1, ncol = 3, common.legend = T, align = 'hv',
          ...
          font.label = list(family = 'Helvetica', size = 10), hjust = list(-1.1,-1.2,-1.1))

ggsave(filename = './Figures/chebi_kb_nmds_times_arranged_landscape_cell_metab.tiff',
       device = 'tiff',
       width = 25.7,
       height = 11.5,
       units = 'cm',
...
# Arrange Cell Metab NMDS -------------------------------------------------
ggarrange(baseline_plot, week_4_plot, week_12_plot, lowcho_plot, 
          nrow = 1, ncol = 4, align = 'hv', labels = NA)
...
## Arrange biplot figures --------------------------------------------------
# Need to run chebi_h2_biplot_nmds_cell_metab.R first
kb_baseline_plot <- baseline_plot
kb_week_4_plot <- week_4_plot
kb_week_12_plot <- week_12_plot

ggarrange(kb_baseline_plot, kb_week_4_plot, kb_week_12_plot,
          h2_baseline_plot, h2_week_4_plot, h2_week_12_plot,
          nrow = 2, ncol = 3, common.legend = T, align = 'hv',
          labels = list('Baseline','Week 4','Week 12','Baseline','Week 4','Week 12'),
          font.label = list(family = 'Helvetica', size = 8), hjust = list(-1.1,-1.3,-1.1))

ggsave(filename = './Figures/chebi_kb_h2_nmds_times_arranged_landscape_cell_metab.tiff',
       device = 'tiff',
       width = 15.55,
       height = 12,
       units = 'cm',
